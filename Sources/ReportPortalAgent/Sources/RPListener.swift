...
public class RPListener: NSObject, XCTestObservation {
  ...
  public func testCaseDidFinish(_ testCase: XCTestCase) {
    queue.async {
      do {
        try
            self.reportingService.finishTest(testCase)
            let data = self.reportingService.readFile()
            try self.reportingService.reportError(message: data)
            self.reportingService.deleteFile()
      } catch let error {
        print(error)
      }
    }
